    ...
    <header className="transition-all absolute z-50 w-full py-4 flex justify-center items-center px-4 sm:px-16">
      <div className="flex justify-between sm:justify-start items-center gap-16 max-w-[1370px]">
        ...
        <div className="relative sm:hidden">
          <button onClick={isMenuOpen ? handleInactive : handleActive} className="flex flex-col justify-center gap-2 w-16 h-16 p-3 bg-white rounded-full relative z-[70]">
            <div
          ...
          <div className={`fixed top-0 left-0 w-screen h-screen bg-primary-dark z-[60] ${isMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none" } transition-opacity duration-500`}>
            <ul className="p-6 w-full h-full space-y-4 text-white text-lg flex flex-col justify-center items-center">
              {
                menuItems?.map(menuItem => (
                  <li onClick={handleInactive} className="overflow-hidden" key={menuItem.title}>
                    <a
                      href={`#${menuItem.path}`}
